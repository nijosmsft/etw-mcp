...
|---|---|---|
| [`rust-vs-csharp-spike-review.md`](rust-vs-csharp-spike-review.md) | Side-by-side review of the Rust and C# extraction-backend spikes; selects C# as the production sidecar language. | Implementation lives in [`../../dotnet/`](../../dotnet/) (originally `csharp/`, renamed to `dotnet/` to match the user-facing `.NET sidecar` label); the review explains the tradeoffs (TraceProcessing parity, MSAL ergonomics, self-contained publish footprint) that made C# the better fit. |
