...
def _make_fake_sidecar(tmp_path: Path) -> Path:
    ...
    target = tmp_path / config.CSHARP_SIDECAR_EXE
...
def _clear_caches(monkeypatch):
    ...
    monkeypatch.delenv(config.CSHARP_SIDECAR_ENV, raising=False)
...
def test_find_csharp_sidecar_honors_env_var(tmp_path, monkeypatch):
    ...
    found = config.find_csharp_sidecar()
...
def test_find_csharp_sidecar_cached(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(config.CSHARP_SIDECAR_ENV, str(sidecar))
    config.reset_csharp_cache()
    first = config.find_csharp_sidecar()
    # Change env, do NOT reset cache: should keep the cached answer.
    monkeypatch.setenv(config.CSHARP_SIDECAR_ENV, str(tmp_path / "other.exe"))
    second = config.find_csharp_sidecar()
    assert first == second
...
def test_resolve_mode_csharp_explicit_missing_binary_raises(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv(config.CSHARP_SIDECAR_ENV, str(tmp_path / "nope.exe"))
    config.reset_csharp_cache()
    ...
    if shutil.which(config.CSHARP_SIDECAR_EXE):
...
def test_resolve_mode_auto_falls_back_to_native_when_csharp_missing(
    ...
    monkeypatch.setenv(config.CSHARP_SIDECAR_ENV, str(tmp_path / "missing.exe"))
    config.reset_auto_cache()
...
def test_auto_detect_ignores_in_tree_csharp_binary(monkeypatch):
    ...
    The behaviour is opt-in via the ``WPR_MCP_CSHARP_SIDECAR`` env var or
    ...
    assert config.find_csharp_sidecar(auto_detect=True) is None
...
def test_explicit_csharp_lookup_includes_in_tree(monkeypatch):
    """The non-auto variant of find_csharp_sidecar checks the in-tree path."""
    ...
    result = config.find_csharp_sidecar(auto_detect=False)
    if result is not None:
        assert result.is_file()
        assert result.name.lower() == config.CSHARP_SIDECAR_EXE.lower()
